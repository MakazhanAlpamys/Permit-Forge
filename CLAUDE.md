...
| `POST /api/permits/[id]/attachments` | Auth | Permit attachment upload (route handler, NOT a server action — bypasses the 1MB `serverActions.bodySizeLimit` so 10MB files upload) |
...
| `lib/gemini.ts` | Gemini model configuration: `chatModel` (temp=0, `maxOutputTokens: 8192`), `streamingModel`, `generateEmbedding()` with retry/quota handling. Model IDs sourced from `lib/llm-config` (v1.7.0 Part F / A-H-8). **`maxOutputTokens` must stay generous: Gemini 2.5 Flash is a thinking model and thinking tokens count against the budget — 4096 truncated the permit-compliance JSON mid-string ("Unterminated string in JSON"). Don't lower it.** |
...
| `lib/csrf-client.ts` | Client-side `readCsrfCookie()` — reads the CSRF token straight from the non-HttpOnly `ef_csrf` cookie (the double-submit cookie is set non-HttpOnly at login precisely so the client can echo it back). **Every client component reads the token from here, NOT via the `getCSRFTokenAction()` server action.** The old server-action round-trip cached the result in a ref/state via a `.catch`-and-log effect; a transient network failure (`ERR_NETWORK_CHANGED` on a VPN/connection flip) left the cached token `null` forever and wedged every subsequent mutation with "CSRF token missing" until a page reload. The synchronous cookie read has no network dependency and can't fail that way. |
...
| `lib/permit-attachment-upload.ts` | Shared permit-attachment upload flow (ownership → state-machine → `validateFile` → Storage `permit-attachments` bucket → atomic `insert_permit_attachment_capped` → audit). Mirrors `lib/document-pdf-upload.ts` / C5H/H6: called by both the `uploadPermitAttachment` server action AND the `/api/permits/[id]/attachments` route so a 10MB upload runs the same validated path without the 1MB Server Action body cap (route handlers don't share it). Caller owns auth/CSRF/rate-limit; `file` is nullable so the "No file provided" check stays after ownership+status (preserves error ordering). |
