...
- [x] **X10 — M4 (clickpath)** Chat Export button race: guard `window.open('/api/chat/export?sessionId=...')` until first message is saved, or add a short post-create delay. `components/chat/chat-interface.tsx:491-503`. **Track B.** 🟢 — (pending commit) — disabled while `isLoading || isStreaming || messages.length === 0`.
