...
- [x] **X9 — L6 (clickpath)** Replace `window.confirm` in `document-management.tsx` with shadcn `<Dialog>` (uses the `<ConfirmDialog>` from F18). **Track F (after F18).** 🟢 — (pending commit) — 4 `confirm()` calls migrated to a shared `<ConfirmDialog>` driven by a `pendingConfirm` state.
