...
- [x] **C5H — H6** Move 100MB body limit from global `next.config.ts` to per-route config for `/api/ingest` only. Restore Next default elsewhere. 🟢 — (commit pending) (moved uploads to /api/admin/documents/upload; server actions back to default 1MB; ingest route gets small JSON body so no per-route cap needed)
