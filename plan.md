...
- [x] **F19 — Simplify #19** Extract `useChatStream()` hook with event-typed SSE protocol. Touches `chat-interface.tsx` AND `app/api/chat/stream/route.ts`. Coordinate with Track B if still open. 🔴 — (pending commit) — extracted `hooks/use-chat-stream.ts` encapsulating the marker-protocol parser + fetch flow. Server-side `__CITATIONS__` / `__ERROR__` markers kept as-is (true typed SSE protocol is a follow-up — out of diploma scope).
