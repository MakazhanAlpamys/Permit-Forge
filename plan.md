...
- [x] **X15 — L8 (clickpath)** Add `revalidatePath('/permits')` / `revalidateTag('permits')` after `reviewPermit` / `setPermitUnderReview` so the user's permit list updates on next mount in other tabs. `actions/admin-permits.ts`. **Track B.** 🟢 — (pending commit) — both actions now `revalidatePath('/permits')` + `/permits/[id]` + `/admin`; `next/cache` mocked globally in `test/setup.ts`.
