...
- [x] **F13 — Simplify #13** Decompose `runIngestionPipeline` into stage functions with auto-computed progress. 🟡 — `3c5ed5f` (split into `loadPdfStage`, `persistKeywordsStage`, `planChunksStage`, `embedAndInsertBatchesStage`; orchestrator now fits on one screen).
...
- [x] **F19 — Simplify #19** Extract `useChatStream()` hook with event-typed SSE protocol. Touches `chat-interface.tsx` AND `app/api/chat/stream/route.ts`. Coordinate with Track B if still open. 🔴 — `31a7378` (extracted `hooks/use-chat-stream.ts`. Kept existing marker protocol — full typed-event SSE is a follow-up out of diploma scope).
- [x] **F20 — Simplify #20** Split `DocumentManagement` into list + form. `useReducer` for form state. Move `BADGE_COLORS` to `lib/constants.ts`. 🟡 — `1a772aa` (extracted `components/admin/document-form.tsx` with `useReducer`; moved colors to `DOCUMENT_BADGE_COLORS` in `lib/constants.ts`).
...
- [x] **X6 — M10 (clickpath)** Add poll (or supabase realtime) on `/permits` so users see status change. **Track B.** 🟢 — `ea60bf6` (60s silent poll, pauses when tab hidden, refreshes on visibility-restore).
- [x] **X7 — L4 (clickpath)** Add jitter to NotificationBell 30s poll. **Track B / F.** 🟢 — `5b1a541` (recursive setTimeout with 30s ± 5s jitter).
- [x] **X8 — L5 (clickpath)** Move `setActionLoading(null)` into a `finally` in `handleDownloadCertificate`. **Track B / F.** 🟢 — `c487bc2`
- [x] **X9 — L6 (clickpath)** Replace `window.confirm` in `document-management.tsx` with shadcn `<Dialog>` (uses the `<ConfirmDialog>` from F18). **Track F (after F18).** 🟢 — `65c5969` (4 `confirm()` calls migrated to a shared `<ConfirmDialog>` driven by `pendingConfirm` state).
- [x] **X10 — M4 (clickpath)** Chat Export button race: guard `window.open('/api/chat/export?sessionId=...')` until first message is saved, or add a short post-create delay. `components/chat/chat-interface.tsx:491-503`. **Track B.** 🟢 — `b2d1c8d` (disabled while `isLoading || isStreaming || messages.length === 0`).
- [x] **X11 — M6 (clickpath)** `CreateUserDialog` flicker: close dialog first, then clear form, then refresh users. `components/admin/create-user-dialog.tsx:46-62`. **Track B / F.** 🟢 — `fce15f0`
- [x] **X12 — M7 (clickpath)** Admin password-change toast: store setTimeout id in a ref and clear on manual close. `app/admin/page.tsx:183-187`. **Track B / F.** 🟢 — `301d70a`
- [x] **X13 — L1 (clickpath)** Chat client cooldown bypass on tab close — persist `lastSentAt` in `sessionStorage` to make `MIN_REQUEST_INTERVAL` survive reload (defense-in-depth; server already catches). `components/chat/chat-interface.tsx:173-178`. **Track B / F.** 🟢 — `0d76114`
- [x] **X14 — L3 (clickpath)** `PermitFormStep3` "Run AI Check" — add client-side guard that requires building details before allowing the call (server already enforces). `components/permits/permit-form-step3.tsx:109-124`. **Track B.** 🟢 — `1328655` (new `hasBuildingDetails` prop; computed from occupancy + construction + ≥1 positive numeric field).
- [x] **X15 — L8 (clickpath)** Add `revalidatePath('/permits')` / `revalidateTag('permits')` after `reviewPermit` / `setPermitUnderReview` so the user's permit list updates on next mount in other tabs. `actions/admin-permits.ts`. **Track B.** 🟢 — `4d78287` (both actions now `revalidatePath('/permits')` + `/permits/[id]` + `/admin`; `next/cache` mocked in `test/setup.ts`).
- [x] **X16 — P2-A2** Extract a central permit state machine (single source of truth for allowed transitions). Replace the 7 duplicated transition checks. Suggested location: `lib/permit-state-machine.ts`. **Track B.** 🟡 — `6f5ad76` (10 typed operations; 8 call-sites migrated across permits/admin-permits/attachments actions, certificate route, and detail page; 12-test suite).
- [x] **X17 — P2-A5** Add `version INT NOT NULL DEFAULT 0` column to `permit_applications`. Every UPDATE bumps `version` and includes `WHERE version = :expected_version`. Two-tab edits → second save returns "permit changed, reload". **Track D (schema) + Track B (action wiring).** 🟡 — (pending commit) — migration 023 adds the column + bumps version in submit/revise/review RPCs. `updatePermitBuildingDetails` + `updatePermitComplianceRequirements` accept optional `expectedVersion`, include it in WHERE, surface "changed in another tab" on mismatch. Client (`/permits/new`) caches `permitVersion` from load + bumps after each successful save. 2 new tests.
- [x] **X18 — P2-A7** Cache-stampede on `semantic_cache` cold start: add an in-process `Map<queryHash, Promise<Result>>` singleflight in `lib/chat-pipeline.ts` so 100 concurrent identical queries collapse to 1 embedding + 1 LLM call. **Track B / F.** 🟡 — `0b1264a` (Map keyed by normalised query, `finally` cleanup; 6-test suite).
