...
- [x] **X14 — L3 (clickpath)** `PermitFormStep3` "Run AI Check" — add client-side guard that requires building details before allowing the call (server already enforces). `components/permits/permit-form-step3.tsx:109-124`. **Track B.** 🟢 — (pending commit) — new `hasBuildingDetails` prop; computed from step 2 occupancy + construction + at least one positive numeric field.
