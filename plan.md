...
- [x] **X17 — P2-A5** Add `version INT NOT NULL DEFAULT 0` column to `permit_applications`. Every UPDATE bumps `version` and includes `WHERE version = :expected_version`. Two-tab edits → second save returns "permit changed, reload". **Track D (schema) + Track B (action wiring).** 🟡 — `77c25d0` (migration 023 adds column + bumps version in submit/revise/review RPCs; update actions accept optional `expectedVersion`; client caches/bumps locally; 2 new tests).
