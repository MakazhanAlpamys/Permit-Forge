...
- [x] **X16 — P2-A2** Extract a central permit state machine (single source of truth for allowed transitions). Replace the 7 duplicated transition checks. Suggested location: `lib/permit-state-machine.ts`. **Track B.** 🟡 — (pending commit) — `lib/permit-state-machine.ts` with 10 operations (`edit`, `upload_attachment`, `delete_attachment`, `run_compliance`, `submit`, `delete`, `revise`, `review`, `start_review`, `download_cert`). Migrated 8 call-sites across `actions/permits.ts`, `actions/admin-permits.ts`, `actions/permit-attachments.ts`, `app/api/permits/[id]/certificate/route.ts`, and `app/permits/[id]/page.tsx`. New 12-test suite in `test/permit-state-machine.test.ts`.
